...
public class EmpQuaController implements OriginController{
	...
	public String indexWithPage(
		...
		model.addAttribute("subMenu", " 就业质量");
	...
	public void testFormBean(@ModelAttribute("update") EmployQuality record,HttpServletResponse response) throws IOException{
//		return JSON.toJSON(sizeService.update(test));
		System.out.println(JSON.toJSON(record));
		if(!empQuaService.update(record)) System.out.println("wrong");;
		response.sendRedirect("./");
		return ;
	} 
