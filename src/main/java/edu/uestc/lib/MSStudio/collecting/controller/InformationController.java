...
public class InformationController implements OriginController{
	...
	public void testFormBean(@ModelAttribute("update") Information test,HttpServletResponse response) throws IOException{
//		return JSON.toJSON(sizeService.update(test));
		System.out.println(JSON.toJSON(test));
		if(!infoService.update(test)) System.out.println("wrong");;
		response.sendRedirect("./");
		return ;
	} 
}
